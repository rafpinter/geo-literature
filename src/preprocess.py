...
class litData:
    ...
    def _total_books_per_country(self):
        ...
        self.books_df = pd.merge(
            self.books_df, df_books_per_country, on="ISO-3", how="left"
        ).fillna(0)
        print(self.books_df.head())

    def _merge_iso_codes_to_equality_df(self):
        self.equality_scores_df = pd.merge(
            self.equality_scores_df,
            self.iso_codes,
            left_on="country",
            right_on="country",
            how="left",
    ...
    def _merge_book_count_to_equality_df(self):
        df_books_per_country = (
            self.books_df[["ISO-3", "books_per_country"]].copy().fillna(0)
        )
        df_books_per_country = df_books_per_country.drop_duplicates()
        self.equality_scores_df = pd.merge(
            self.equality_scores_df, df_books_per_country, on="ISO-3", how="left"
        ).fillna(0)
